...
void OSPRayVolleyTestScene::setIsovalues(const std::vector<float> &isovalues)
{
...
OSPModel OSPRayVolleyTestScene::getWorld()
{
  return world;
}

OSPRenderer OSPRayVolleyTestScene::getRenderer()
{
  return renderer;
}

OSPTransferFunction OSPRayVolleyTestScene::getTransferFunction()
{
  return transferFunction;
}

box3f OSPRayVolleyTestScene::getBoundingBox()
{
  vly_box3f boundingBox = vlyGetBoundingBox(proceduralVolume->getVLYVolume());
  return reinterpret_cast<box3f &>(boundingBox);
}

std::unique_ptr<OSPRayWindow> OSPRayVolleyTestScene::getOSPRayWindow(
