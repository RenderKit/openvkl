...
namespace volley {
  ...
  template <typename T, VLYDataType VLY_TYPE = VLY_UNKNOWN>
  inline T *objectFactory(const std::string &type)
  {
    ...
    const auto type_string = stringForType(VLY_TYPE);
    ...
    if (symbolRegistry.count(type) == 0) {
      ...
      std::string creationFunctionName =
          "volley_create_" + type_string + "__" + type;
    ...
    if (object == nullptr) {
      ...
      throw std::runtime_error(
          ...
          ".  Make sure you have the correct Volley libraries linked.");
    }
